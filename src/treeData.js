  ...
  const result = keys.map((currentKey) => {
    if (_.has(data1, currentKey) && _.has(data2, currentKey)) {
      if (_.isObject(data1[currentKey]) && _.isObject(data2[currentKey])) {
        return { key: currentKey, children: getTreeData(data1[currentKey], data2[currentKey]), type: 'children' };
      }
    ...
    if (!_.has(data1, currentKey)) {
      return { key: currentKey, value: data2[currentKey], type: 'add' };
    }
    return { key: currentKey, value: data1[currentKey], type: 'del' };
  });
  return result;
};
